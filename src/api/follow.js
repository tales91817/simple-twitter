...
export const postFollower = async(user_id) => {
    try {
        ...
        const response = await axios.post( `${baseUrl}/api/followships`, { 
            id: user_id
        }, { headers })
...
export const deleteFollower = async(user_id) => {
    try {
        const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTQsImFjY291bnQiOiJ1c2VyMCIsImVtYWlsIjoidXNlcjBAZXhhbXBsZS5jb20iLCJuYW1lIjoiRXJpa2EgRnJpdHNjaCIsImF2YXRhciI6Imh0dHBzOi8vbG9yZW1mbGlja3IuY29tLzE0MC8xNDAvZm9vZC8_.GwX_wkXUJXAsGNq4kKov6q_xVjA0-_tr6OUd0PqoF20'
        const headers = {
            Authorization: `Bearer ${token}`
        ...
        localStorage.setItem('token', token);
        const response = await axios.delete( `${baseUrl}/api/followships/${user_id}`, { headers })
        return response.data 
...
export const getFollower = async(user_id) => {
    try {
        ...
        localStorage.setItem('token', token);
        const response = await axios.get( `${baseUrl}/api/users/${user_id}/followers`, { headers })
        return response.data 
...
export const getFollowing = async(user_id) => {
    try {
        ...
        localStorage.setItem('token', token);
        const response = await axios.get( `${baseUrl}/api/users/${user_id}/followings`, { headers })
        return response.data 
