import axios from 'axios';
const baseUrl = 'https://afternoon-waters-66047.herokuapp.com';

export const getAllReplies = async() => {
    try {
        const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTQsImFjY291bnQiOiJ1c2VyMCIsImVtYWlsIjoidXNlcjBAZXhhbXBsZS5jb20iLCJuYW1lIjoiRXJpa2EgRnJpdHNjaCIsImF2YXRhciI6Imh0dHBzOi8vbG9yZW1mbGlja3IuY29tLzE0MC8xNDAvZm9vZC8_.GwX_wkXUJXAsGNq4kKov6q_xVjA0-_tr6OUd0PqoF20'
        const headers = {
            Authorization: `Bearer ${token}`
        }

        localStorage.setItem('token', token);
        const response = await axios.get( `${baseUrl}/api/tweets/14/replies`, { headers })
        return response.data

    } catch (error) {
        console.error(error)
    }
};

export const replyToTweet = async(payload) => {
    const { comments } = payload
    try {
        const token = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpZCI6MTQsImFjY291bnQiOiJ1c2VyMCIsImVtYWlsIjoidXNlcjBAZXhhbXBsZS5jb20iLCJuYW1lIjoiRXJpa2EgRnJpdHNjaCIsImF2YXRhciI6Imh0dHBzOi8vbG9yZW1mbGlja3IuY29tLzE0MC8xNDAvZm9vZC8_.GwX_wkXUJXAsGNq4kKov6q_xVjA0-_tr6OUd0PqoF20'
        const headers = {
            Authorization: `Bearer ${token}`
        }

        localStorage.setItem('token', token);
        const response = await axios.post( `${baseUrl}/api/tweets/14/replies`, { 
            comments
        }, { headers })
        return response.data 

    } catch(error) {
        console.error(error)
    }
}